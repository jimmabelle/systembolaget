...
$result = mysqli_query($conn, "SELECT COUNT(*) as count FROM admin"); 	
	while ($row = mysqli_fetch_array($result)) {
	 $userName3 = $row['count'];
		echo "We have " .$userName3. " registered admin users.<br>";	
...
if (isset($_POST['register'])) {
		...
		$dbSql = ("SELECT * FROM admin WHERE  epost='".$epost."'");
		...
		$dbNumRows = mysqli_num_rows($dbResult);
			...
			if ($dbNumRows > 0) {
				echo 'Ditt konto finns redan!';
				 ...
				 $insertSql = ("INSERT INTO admin (name, epost, password, rePassword) VALUES ('".$name."', '".$epost."', '".$password1."', '".$password2."')");
